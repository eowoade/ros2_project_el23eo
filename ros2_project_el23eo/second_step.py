...
class colourIdentifier(Node):
    ...
    def callback(self, data):
        ...
        green_mask = cv2.inRange(image, hsv_green_lower, hsv_green_upper)
        red_mask = cv2.inRange(image, hsv_red_lower, hsv_red_upper)
        blue_mask = cv2.inRange(image, hsv_blue_lower, hsv_blue_upper)
        rg_mask = cv2.bitwise_or(green_mask, red_mask)
        mask = cv2.bitwise_or(rg_mask, blue_mask)
